...
class LearningTests(unittest.TestCase):
    ...
    def test_analyze_logs_builds_model_bias(self):
        ...
        self.assertEqual(state["analytics"]["event_count"], 2)
        self.assertEqual(state["recommendations"]["model"]["sync_metadata"]["recommended_model"], "local")
        self.assertEqual(state["recommendations"]["model"]["software_engineering"]["recommended_model"], "codex")
    ...
    def test_analyze_logs_uses_feedback_for_model_bias(self):
        ...
        self.assertEqual(state["recommendations"]["model"]["sync_metadata"]["recommended_model"], "codex")
        self.assertEqual(state["analytics"]["intents"]["sync_metadata"]["models"]["codex"], 1)
    ...
    def test_update_learning_state_handles_missing_log_file(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertEqual(state["analytics"]["event_count"], 0)
    ...
    def test_load_learning_state_reads_valid_state_dict(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            state_path.write_text(json.dumps({
                "recommendations": {"model": {}, "agent": {}, "confidence": {}},
                "analytics": {"event_count": 3, "intents": {}, "models": {}, "agents": {}, "low_confidence_patterns": []}
            }))

            state = load_learning_state(str(state_path))

            self.assertEqual(state["analytics"]["event_count"], 3)
    ...
    def test_get_model_bias_for_intent(self):
        ...
        self.assertEqual(get_model_bias_for_intent("sync_metadata", state)["recommended_model"], "local")
    ...
    def test_update_learning_state_reads_log_file(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertEqual(state["analytics"]["event_count"], 1)
            self.assertEqual(state["recommendations"]["model"]["system_architecture"]["recommended_model"], "claude")
